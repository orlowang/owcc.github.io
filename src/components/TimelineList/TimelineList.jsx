...
class TimelineList extends Component {
	...
	render() {
		...
		var child = _posts.map(function(post){
			if (typeof post != 'object') {
				return (
					<div key={post} className="yearland">
				...
				return (
					<div key={post.id} className="listcard cfix" style={post.preview != '' ? { height: '15.31vw' } : {}}>
						<div className="doctext" style={{ width: post.preview != '' ? '61.2%' : '100%' }}>
							<Link query={{ get: post.id }} to='/'>{post.title}</Link>
							<p className="text">{post.subtitle}</p>
							<div className="date">Post time: <strong>{month}</strong>{tmp$time[2]}</div>
						</div>
